...
TENSORS_TO_LOG = {
    ...
    "concrete_loss" : "model/concrete_loss",
    "total_loss" : "model/total_loss",
    "concrete_reg" : "model/concrete_reg",
    "sparsity_rate": "model/sparsity_rate",
...
def get_learning_rate(learning_rate, hidden_size, learning_rate_warmup_steps, checkpoint_step = 0):
  """Calculate learning rate with linear warmup and rsqrt decay."""
  with tf.name_scope("learning_rate"):
    warmup_steps = tf.to_float(learning_rate_warmup_steps)
    step = tf.to_float(tf.train.get_or_create_global_step()) - tf.to_float(checkpoint_step)
    step = tf.to_float(tf.math.floordiv(tf.cast(step, dtype=tf.int64), tf.constant(10, dtype=tf.int64)))
...
def get_train_op_and_metrics(loss, params):
  print(" **** Accumulate gradients is = {} ****".format(accumulate_grad))
  ...
  with tf.variable_scope("get_train_op"):
    ...
    if accumulate_grad == True:
      tf.logging.info('accgrad')
      minimize_op = optimizer.apply_gradients([(accum_vars[i], gv[1]) for i, gv in enumerate(gradients)], global_step=global_step, name="train")
      with tf.get_default_graph().control_dependencies([minimize_op]):
          zero_ops = [tv.assign(tf.zeros_like(tv)) for tv in accum_vars]
      train_op = tf.cond(tf.math.equal(tf.math.floormod(global_step, tf.constant(10, dtype=tf.int64)),0), lambda: tf.group(minimize_op, update_ops, zero_ops), lambda: tf.group(accum_ops, update_ops))
    else:
      tf.logging.info('fromhere')
...
def define_transformer_flags():
  ...
  flags.DEFINE_string(
      name="concrete_heads", default="['enc-self','dec-self','enc-dec']",
      help=flags_core.help_wrap(
          "Which type of gates to be learnt : enc-self/dec-self/dec-enc"
      )
  )
  flags.DEFINE_float(
      name="concrete_coef", default=0.0,
      help=flags_core.help_wrap(
          "Concrete coefficient"
      )
  )
  flags.DEFINE_string(
      name="alive_heads_enc_self", default="[]",
      help=flags_core.help_wrap(
          "Fixed gate values for enc-self attention"
      )
  )
  flags.DEFINE_string(
      name="alive_heads_dec_self", default="[]",
      help=flags_core.help_wrap(
          "Fixed gate values for dec-self attention"
      )
  )
  flags.DEFINE_string(
      name="alive_heads_enc_dec", default="[]",
      help=flags_core.help_wrap(
          "Fixed gate values for dec_enc attention"
      )
  )

  flags.DEFINE_float(
      name="checkpoint_step", default=0.0,
      help=flags_core.help_wrap(
          "Checkpoint Step"
      )
  )
...
def construct_estimator(flags_obj, params, schedule_manager):
  ...
  if not params["use_tpu"]:
    ...
    return tf.estimator.Estimator(
        ...
        config=tf.estimator.RunConfig(train_distribute=distribution_strategy,keep_checkpoint_max=5,save_checkpoints_steps=76135))
...
def run_transformer(flags_obj):
  ...
  params["checkpoint_step"] = flags_obj.checkpoint_step

  #params["apply_gates"] = flags_obj.apply_gates
  params["concrete_coef"] = flags_obj.concrete_coef
  params["concrete_heads"] = eval(flags_obj.concrete_heads)
  params["alive_heads_enc_self"] = eval(flags_obj.alive_heads_enc_self)
  params["alive_heads_dec_self"] = eval(flags_obj.alive_heads_dec_self)
  params["alive_heads_enc_dec"] = eval(flags_obj.alive_heads_enc_dec)
